...
class _LoginPageState extends State<LoginPage> {
  ...
  void initState() {
    ...
    if (context.read<ServerBloc>().state.genders.isNotEmpty) {
      ...
      for (GenderModel gender in context.read<ServerBloc>().state.genders) {
        genders.addAll({gender.id: gender.name});
      }
    }
    if (context.read<ServerBloc>().state.countries.isNotEmpty) {
      countries = {};
      countries.addAll({"": "Select country"});
      for (CountryModel country in context.read<ServerBloc>().state.countries) {
  ...
  Widget display() {
    return MultiBlocListener(
      listeners: [
        ...
        BlocListener<ServerBloc, ServerState>(
          listenWhen: (previous, current) => previous.countries != current.countries,
          listener: (context, state) {
            setState(() {
